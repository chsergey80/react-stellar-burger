...
* 34 commit / Исправление замечаний по результату код-ревью (Можно лучше: лучше вынести в отдельный компонент заголовок и содержимое раздела в файле src/components/burger-ingredients/burger-ingredients.jsx стр.51) - оставлено без изменений из-за стремления оптимизировать код.
            / Исправление замечаний по результату код-ревью (Можно лучше: табы тоже можно вынести в отдельный комопнент в файле src/components/burger-ingredients/burger-ingredients.jsx стр.42-44) - оставлено без изменений из-за стремления оптимизировать код.
            / Исправление замечаний по результату код-ревью (Можно лучше: фильтры лучше вынести в useMemo в файле src/components/burger-ingredients/burger-ingredients.jsx стр.25-27) - исправлено.
* 35 commit / Исправление замечаний по результату код-ревью (Можно лучше: вместо querySelector лучше получать элемент с помощью useRef в файле src/components/burger-ingredients/burger-ingredients.jsx) - будет учтено в ближайшей доработке проекта.
            / Исправление замечаний по результату код-ревью (Можно лучше: в селекторе, по возможности лучше выбирать только то, что вам нужно, чтобы не инициализировать лишних обновлений (в файле src/components/burger-ingredients/burger-ingredients.jsx). В данно случае const data = useSelector(store => store.data.data)) - будет учтено в ближайшей доработке проекта.
            / Исправление замечаний по результату код-ревью (Можно лучше: стоит придумать более понятное название вместо data, чтобы оно описывало содержимое. А то не совсем понятно что это такое) - будет учтено в ближайшей доработке проекта.
            / Исправление замечаний по результату код-ревью (Можно лучше: Используйте Prettier для автоматического форматирования вашего когда) - будет учтено в ближайшей доработке проекта после изучения этой технологии более детально.
            / Исправление замечаний по результату код-ревью (Можно лучше: madalReducer -> modalReduce в файле src/services/reducers/burger-ingredients.js) - исправлено.
* 36 commit / Исправление замечаний по результату код-ревью (Можно лучше: можно сократить const getIngredients = () => request(${config.urlIngredients}) в файле src/utils/burger-api.js) - исправлено.